...
const page = (props: Props) => {
    ...
    const { data: allCardsOfSet, isLoading: isLoadingAllCardsOfSet } = useGetCardBySetIdQuery(id, {});
    ...
    const handleMultipleChoiceSubmit = async () => {
        ...
        const isCorrect = answerChoicesData?.correctAnswerId  == selectedAnswer || false;
    ...
    const handleStartLearning = async () => {
        ...
        setTotalCorrect(allCardsOfSet?.data?.length - response?.data?.remainingCards?.length);
        // setCards(allCardsOfSet?.data);
        const remainingCardIds = new Set(response?.data?.remainingCards.map((card: any) => card.id));
        console.log("remainingCardIds", remainingCardIds)
        const allCards = allCardsOfSet?.sets;
        setCards(allCards);
        const answeredCards = allCards.filter((card: any) => !remainingCardIds.has(card.id));
                                ...
                                <Card className="w-full max-w-2xl p-6">
                                    <div className="flex justify-between items-center mb-6">
                                        <h3 className="text-lg font-semibold">
                                            Question {answeredCards?.length} of {cards?.length}
                                        </h3>
                                    ...
                                    {isLoadingAnswerChoices ? (
                                        ...
                                        >
                                            {answerChoicesData?.choices.map((choice: any) => {
                                                ...
                                                const isCorrect = answerChoicesData.correctAnswerId === choice.id;
